...
var drawDot = function drawDot(ctx, pos, sf) {
    var dotSize = sf.dotSize * 30;
    ctx.closePath();
    ctx.stroke();
    ctx.beginPath();
...
var draw = exports.draw = function draw(ctx, sf, startPos) {
    ...
    var deltaStep = PI2 * sf.angle + time * sf.angleSpeed / 200;
    var hDeltaStep = deltaStep / 2;

    var startSize = sf.scale * window.innerHeight;
    var bigRot = sf.treeRot * Math.PI;
    var dotThreshold = Math.floor(sf.numSteps * (1 - sf.dotThreshold));
    var lineThreshold = Math.floor(sf.numSteps * sf.lineThreshold);

    ctx.moveTo(startPos[0], startPos[1]);

    var branch = function branch(numBranches, pos, distance, delta, treeIndex) {
        if (numBranches < sf.numSteps) {
            ...
            var posDelta = delta + deltaStep + sf.bend;
            var negDelta = delta - deltaStep + sf.bend;

            var posL = [eX + pos[0] + Math.sin(delta + posDelta - hDeltaStep) * distance, eY + pos[1] + Math.cos(delta + posDelta - hDeltaStep) * distance];

            var posR = [eX + pos[0] + Math.sin(delta + posDelta + hDeltaStep) * distance, eY + pos[1] + Math.cos(delta + posDelta + hDeltaStep) * distance];
    ...
    var rotStep = PI2 / sf.numTrees / 2;

    for (var i = 0; i < sf.numTrees; i++) {
        var newStartPos = [startPos[0] + Math.sin(-rotStep * i * 2) * sf.startRad, startPos[1] + Math.cos(-rotStep * i * 2) * sf.startRad];

        var startRot = -rotStep * i + bigRot;
